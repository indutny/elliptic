...
describe('ed25519 derivations', function () {
  ...
  function testFactory(i) {
    it('can compute correct a and A for secret: ' + i, function () {
      ...
      var xRecovered = toHex(
        ed25519.encodeInt(ed25519.decodePoint(key.pubBytes()).getX()),
      );
    ...
    it('vector ' + i, function () {
      ...
      var forged =
        msg.length === 0
          ? [ 0x78 ] /*ord('x')*/
          : msg
            .slice(0, msg.length - 1)
            .concat((msg[msg.length - 1] + 1) % 256);

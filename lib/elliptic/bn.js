...
BN.prototype.toMont = function toMont(ctx) {
  ...
  var res = this.mul(ctx.r).mod(ctx.m);
  res.mont = ctx;
  return res;
};
...
BN.prototype.montShl = function montShl(num) {
  assert(!this.sign, 'mont works only with positives');
  assert(this.mont, 'montMul works only with mont numbers');
...
BN.prototype.montInvp = function montInvp(num, base) {
  // Exponent to `num-1`, assuming `num` is prime
};

...
BN.prototype.init = function init(number, base) {
  if (typeof number === 'number') {
    ...
    // Perhaps a Buffer or an Uint8Array
    assert(typeof Buffer !== 'undefined' ?
              number instanceof Buffer :
              typeof Uint8Array !== 'undefined' &&
                  number instanceof Uint8Array);
    this.length = Math.ceil(number.length / 2);
    this.words = new Array(this.length);

    if (base === 'little' || base === 'le') {
      for (var i = 0; i < this.length; i++) {
        this.words[this.length - i - 1] =
            number[i * 2] | (number[i * 2 + 1] << 8);
      }
    } else {
      for (var i = 0; i < this.length; i++)
        this.words[i] = number[i * 2] | (number[i * 2 + 1] << 8);
    }
    return;
  }
  assert(base <= 16);
...
BN.prototype.iadd = function iadd(num) {
  ...
  // Carry
  for (var i = 0; i < this.length && this.words[i] >= 0x10000; i++) {

...
Point.prototype.mulAdd = function mulAdd(k1, p2, k2) {
  ...
  // Convert everything to J
  for (var i = 0; i < wnd.a.length; i++)
    wnd.a[i] = wnd.a[i].toJ();
  ...
  for (var i = Math.max(naf.b.length - 1, naf.a.length - 1); i >= 0; i--) {
    ...
    var z = naf.a[i] | 0;
    if (z > 0)
      acc = acc.add(wnd.a[(z - 1) >> 1]);
    else if (z < 0)
      acc = acc.add(wnd.a[(-z - 1) >> 1].neg());
    var z = naf.b[i] | 0;
    if (z > 0)
      acc = acc.add(wnd.b[(z - 1) >> 1]);
    else if (z < 0)
      acc = acc.add(wnd.b[(-z - 1) >> 1].neg());
  }
  return acc.toP();
};

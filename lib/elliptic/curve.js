...
Curve.prototype._fixedNafMul2 = function _fixedNafMul2(p1, k1, p2, k2) {
  var doubles = {
    p1: p1._getDoubles(),
    p2: p2._getDoubles()
  };
  assert.equal(doubles.p1.step, doubles.p2.step);
  assert(!k1.sign);

  var step = doubles.p1.step;

  // Negate all cached doubles
  if (k2.sign) {
    k2 = k2.neg();
    p2 = p2.neg();
    var points = [];
    for (var i = 0; i < doubles.p2.points.length; i++)
      points[i] = doubles.p2.points[i].neg();
    doubles.p2 = {
      step: step,
      points: points
    };
  ...
  for (var j = 0; j < naf.p1.length; j += step) {
    var nafW = 0;
    for (var k = j + step - 1; k >= j; k--)
      nafW = (nafW << 1) + naf.p1[k];
    repr.p1.push(nafW);
  }
  for (var j = 0; j < naf.p2.length; j += step) {
    var nafW = 0;
    for (var k = j + step - 1; k >= j; k--)
      nafW = (nafW << 1) + naf.p2[k];
    repr.p2.push(nafW);
  }

  var I = (1 << (step + 1)) - (step % 2 === 0 ? 2 : 1);
  I /= 3;

  var a = this.jpoint(null, null, null);
  var b = this.jpoint(null, null, null);
  var max = Math.max(repr.p1.length, repr.p2.length);
  for (var i = I; i > 0; i--) {
    for (var j = 0; j < max; j++) {
      var nafW = repr.p1[j];
      if (nafW === i)
        b = b.mixedAdd(doubles.p1.points[j]);
      else if (nafW === -i)
        b = b.mixedAdd(doubles.p1.points[j].neg());
      var nafW = repr.p2[j];
      if (nafW === i)
        b = b.mixedAdd(doubles.p2.points[j]);
      else if (nafW === -i)
        b = b.mixedAdd(doubles.p2.points[j].neg());
    }
    a = a.add(b);
  }
  return a.toP();
...
Curve.prototype._endoMul = function _endoMul(p, k) {
  var split = this._endoSplit(k);
  var betaP = p._getBeta();
  ...
  if (k1.sign) {
    k1.sign = !k1.sign;
    k2.sign = !k2.sign;
    return this._fixedNafMul2(p, k1, betaP, k2).neg();
  } else {
    return this._fixedNafMul2(p, k1, betaP, k2);
  }
};
...
Point.fromJSON = function fromJSON(curve, obj, red) {
  ...
  var pre = obj[2];
  res.precomputed = {
    beta: pre.beta && Point.fromJSON(curve, pre.beta, red),
    doubles: pre.doubles && {
...
Point.prototype.precompute = function precompute(power, order, _beta) {
  ...
  precomputed.beta = _beta ? null : this._getBeta();
  if (precomputed.beta)
    precomputed.beta.precompute(power, order, true);
...
Point.prototype._getBeta = function _getBeta() {
  ...
  return this.curve.point(this.x.redMul(this.curve.endo.beta), this.y);
};
...
Point.prototype.mul = function mul(k) {
  ...
  if (this.precomputed && this.curve.endo)
    return this.curve._endoMul(this, k);
  else if (this.precomputed && this.precomputed.doubles)

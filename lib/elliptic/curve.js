...
Point.fromJSON = function fromJSON(curve, obj, red) {
  ...
  if (!obj[2])
    return res;
  res.precomputed = {
    doubles: obj[2].doubles && obj[2].doubles.map(function(obj) {
      return curve.point(obj[0], obj[1], red);
    }),
    naf: obj[2].naf && {
      wnd: obj[2].naf.wnd,
      points: [ res ].concat(obj[2].naf.points.map(function(obj) {
        return curve.point(obj[0], obj[1], red);
      }))
    }
  };
...
Point.prototype.precompute = function precompute(power) {
  ...
  for (var i = 0; i < power; i++) {
    var q = (i === 0 ? this : precomputed.doubles[i - 1]).dbl();
    precomputed.doubles.push(q);
  }
...
Point.prototype.inspect = function inspect() {
  ...
  return '<EC Point x: ' + this.x.fromRed().toString(16) +
      ...
      ' pc: ' + (this.precomputed && this.precomputed.doubles.length || 0) +
      '>';
};
...
Point.prototype.dbl = function dbl() {
  ...
  if (this.precomputed && this.precomputed.doubles) {
    var cached = this.precomputed.doubles[0];
    var res = this.curve.point(cached.x, cached.y);
    if (this.precomputed.doubles.length > 1) {
      res.precomputed = {
        doubles: this.precomputed.doubles.slice(1),
        naf: null
      };
    }
    return res;
...
Point.prototype._naiveMulAdd = function _naiveMulAdd(k1, p2, k2) {
  var a = this;
  var b = p2;
  assert.equal(b.type, 'affine');
  ...
  if (a.precomputed && a.precomputed.length) {
    if (a.precomputedN) {
      wnd.a = a.precomputedN;
    } else {
      for (var i = 1; i < (1 << w) - 1; i++)
        wnd.a[i] = wnd.a[i - 1].add(a);
      a.precomputedN = wnd.a;
    }
  } else {
    for (var i = 1; i < (1 << w) - 1; i++)
      wnd.a[i] = wnd.a[i - 1].add(a);
  ...
  k2 = k2.clone();
  while (k1.cmpn(0) !== 0) {
    bform.a.push(k1.andln(mask));
    k1.ishrn(w);
  }
  while (k2.cmpn(0) !== 0) {
    bform.b.push(k2.andln(mask));
    k2.ishrn(w);
  }

  var acc = this.curve.jpoint(null, null, null);
  for (var i = Math.max(bform.a.length, bform.b.length) - 1; i >= 0; ) {
    var k = 0;
    do {
      var za = bform.a[i] | 0;
      var zb = bform.b[i] | 0;
    ...
    if (za !== 0)
      acc = acc.mixedAdd(wnd.a[za - 1]);
    if (zb !== 0)
      acc = acc.mixedAdd(wnd.b[zb - 1]);
  }
  return acc.toP();
};

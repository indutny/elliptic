...
EdwardsCurve.prototype.point = function point(x, y, z) {
  return new Point(this, x, y, z);
};

// Just for compatibility with Short curve
EdwardsCurve.prototype.jpoint = function jpoint(x, y, z) {
...
Point.prototype.dbl = function dbl() {
  ...
  if (this.curve.twisted) {
    ...
    var e = c.redAdd(d);
    // H = (c * Z1)^2
    var h = this.curve.c.redMul(this.z).redSqr();
    // J = E - 2 * H
    var j = e.redSub(h).redSub(h);
    // X3 = c * (B - E) * J
    var nx = this.curve.c.redMul(b.redISub(e)).redMul(j);
    // Y3 = c * E * (C - D)
    var ny = this.curve.c.redMul(e).redMul(c.redISub(d));
    // Z3 = E * J
    var nz = e.redMul(j);
  }
...
Point.prototype.add = function add(p) {
  ...
  if (this.curve.twisted) {
    // Y3 = A * G * (D - a * C)
    var ny = a.redMul(g).redMul(d.redSub(this.curve.a.redMul(c)));
    // Z3 = F * G
    var nz = f.redMul(g);
  } else {
    // Y3 = A * G * (D - C)
    ...
    // Z3 = c * F * G
    var nz = this.curve.c.redMul(f).redMul(g);
  }
  return this.curve.point(nx, ny, nz);
};
...
Point.prototype.neg = function neg() {
  return this.curve.point(this.x.redNeg(), this.y, this.z);
};
